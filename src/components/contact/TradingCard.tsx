...
export function TradingCard({
  ...
  const pad = dense ? 'p-6' : 'p-4';
  const aspect = dense ? 'aspect-[270/380] h-full' : 'aspect-[270/380]';
  const avatar = dense ? 'w-28 h-28 rounded-3xl text-3xl' : 'w-20 h-20 rounded-2xl text-2xl';
  const h3 = dense ? 'text-2xl' : 'text-lg';
  const company = dense ? 'text-sm' : 'text-xs';
  const gapY = dense ? 'gap-4' : 'gap-3';
  ...
  return (
    <motion.div
      ...
      className={[
        `relative ${aspect} w-full rounded-2xl overflow-hidden select-none`,
    ...
    >
      <div
        ...
        style={{
          padding: '1px',
          background: `linear-gradient(${reduced ? 135 : 'var(--hl-angle, 135deg)'} at ${reduced ? '30% 20%' : 'var(--hl-px, 30%)'} ${reduced ? '20%' : 'var(--hl-py, 20%)'}, rgba(255,255,255,0.18), rgba(255,255,255,0) 50%)`,
          WebkitMask:
      ...
      <div
        aria-hidden="true"
        className="card-foil pointer-events-none absolute inset-0 rounded-2xl opacity-70"
        style={{
      ...
      <motion.div
        ...
        className={`relative h-full w-full ${pad} flex flex-col ${gapY}`}
        style={{ zIndex: 2 }}
      >
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h3 className={`mt-3 font-display font-bold text-white leading-tight ${h3}`}>
            {contact.firstName} {contact.lastName}
          </h3>
          {contact.company && (
            <p className={`${company} text-slate-400 mt-0.5 truncate max-w-full px-2`}>
              {contact.company}
            </p>
          )}
        </div>
          ...
          {activeRequest ? (
            <>
              <div>
                ...
                <p className={`${dense ? 'text-base' : 'text-sm'} font-medium text-slate-100 truncate`}>
                  {activeRequest.title}
                </p>
              </div>
              ...
              {activeRequest.nextAction && (
                <NextActionView action={activeRequest.nextAction} variant={dense ? 'standard' : 'standard'} />
              )}
            </>
          ) : (
            <div className={`${dense ? 'text-sm' : 'text-xs'} text-slate-500 italic text-center py-3`}>
        ...
        <div className="mt-auto">
          ...
          <div className={`flex items-center justify-between ${dense ? 'text-xs' : 'text-[11px]'} text-slate-400`}>
