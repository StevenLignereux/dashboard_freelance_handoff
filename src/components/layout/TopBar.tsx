...
export function TopBar({ onToggleSidebar, menuButtonRef }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-20 h-16 shrink-0">
      <div className="h-full px-4 sm:px-6 lg:px-8 flex items-center gap-3 md:gap-4 bg-bg/70 backdrop-blur-xl border-b border-brand-violet/10">
        <button
          type="button"
          ref={menuButtonRef}
        ...
        <div className="relative flex-1 max-w-2xl">
          ...
          <input
            ...
            className="w-full h-10 pl-10 pr-24 rounded-xl bg-bg-surface/70 border border-brand-violet/10 text-sm text-slate-800 placeholder:text-slate-400
          ...
          <div className="pointer-events-none absolute inset-y-0 right-2 flex items-center gap-1 text-slate-400">
            <kbd className="px-1.5 py-0.5 text-[10px] font-semibold rounded-md border border-brand-violet/20 bg-brand-violet/5">
              ⌘
            </kbd>
            <kbd className="px-1.5 py-0.5 text-[10px] font-semibold rounded-md border border-brand-violet/20 bg-brand-violet/5">
              K
        ...
        <div className="ml-1 sm:ml-2 pl-2 sm:pl-3 border-l border-brand-violet/10 flex items-center gap-3">
          <div className="hidden sm:block text-right leading-tight">
            <div className="text-sm font-semibold text-slate-900">{currentUser.firstName}</div>
