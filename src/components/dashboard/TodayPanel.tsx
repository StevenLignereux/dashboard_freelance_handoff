import { dashboardTodayItems } from '../../data/mockData';
...
export function TodayPanel({ onOpenContact }: TodayPanelProps) {
  const count = dashboardTodayItems.length;
      ...
      <div className="relative">
        <header className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            <span className="w-8 h-8 rounded-xl bg-brand-violet/15 text-brand-violet ring-1 ring-brand-violet/30 flex items-center justify-center">
            ...
            <h2 className="font-display font-bold text-white text-lg leading-none">
              À faire aujourd&rsquo;hui
            </h2>
          </div>
          <span className="chip bg-brand-violet/15 text-brand-violet ring-1 ring-brand-violet/30">
            {count}
          </span>
        </header>
          ...
          {dashboardTodayItems.map((item) => {
            ...
            return (
              <li key={item.id}>
                ...
                >
                  <span className="w-8 h-8 shrink-0 rounded-full bg-brand-violet/15 text-brand-violet ring-1 ring-brand-violet/30 flex items-center justify-center">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {hour && (
                      <div className="text-[11px] text-brand-violet font-medium mt-0.5 flex items-center gap-1">
